...
Скопировано из старого ``ebaylib`` (SPEC.md §12). Единственный источник URL поиска:
артикул + страница + опциональные фильтры (ZIP доставки, состояние, цена).
Воркер импортирует ``build_search_url`` и сам решает значения фильтров.
"""
