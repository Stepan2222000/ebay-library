...
Политика блоков (реализуется в navigation, не здесь):
  PARDON        → ждём (JS-челлендж проходит сам), таймаут 3 минуты;
  ERROR_PAGE    → транзиентный сбой, retryable;
  ACCESS_DENIED → воркер помирает (перезапуск другого — out of scope).
"""
...
class Antibot(str, Enum):
    ...
    # Транзиентная статическая страница "SORRY — Something went wrong on our end"
    # (<title>Error Page | eBay, robots=noindex, reference-id, без JS-челленджа).
    # Появляется без прогрева. Пока только детектируем, обработку решим позже.
...
def detect_antibot(title: str | None) -> Antibot | None:
    ...
    Access Denied проверяем первым (жёсткий блок), затем Error Page
    (транзиентный сбой), затем Pardon (мягкий челлендж). Обработку каждого
    решает слой навигации — здесь только различаем.
    """
    if not title:
        return None
    low = title.lower()
