...
(сеть/таймаут/5xx) → исключение httpx наружу. Сбой fx критичен — валит
задачу целиком (заменой страницы не лечится, это не браузерная проблема).
...
async def convert_cards(cards: list[SrpCard], *, base_url: str = FX_API_URL) -> list[CatalogItem]:
    ...
    обратно по карточкам. Free (0.0) и отсутствующая (None)
    доставка не конвертируются — остаются как есть. Любой сбой fx пробрасывается наружу."""
    if not cards:
        return []
